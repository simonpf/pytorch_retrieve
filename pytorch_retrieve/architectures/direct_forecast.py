"""
pytroch_retrieve.architectures.direct_forecast
==============================================

Direct forecast models produce their forecast based on a lead-time encoded
in the input.
"""
...
def encode_times(lead_times, time_step, forecast_range):
    """
    1h encoder lead time.

    Args:
        lead_times: Tensor containing the lead time to encode.
        time_step: The forecast time step.
        forecast_range: The maximum range of the forecast.

    Return:
        A tensor of shape (batch_size * n_forecasts, n_times) containing
        the 1-hot encodings of the lead times.
    """
    lead_times = torch.minimum(
        lead_times, torch.tensor(forecast_range)
    ) / time_step
    n_times = forecast_range // time_step
    lead_times = nn.functional.one_hot(
        lead_times.to(dtype=torch.int64) - 1,
        num_classes=n_times
    )
    lead_times = lead_times.reshape(-1, n_times)
    return lead_times
...
class TemporalEncoderConfig:
    ...
    @classmethod
    def parse(cls, latent_dim: int, n_times: int, config_dict: Dict[str, Any]) -> "TemporalEncoderConfig":
        """
        ...
        channels = get_config_attr(
            "channels", list, config_dict, "architecture.temporal_encoder"
        )
        block_factory = get_config_attr(
            "block_factory", str, config_dict, "architecture.temporal_encoder", "BasicConv"
        )
        block_factory_args = get_config_attr(
            "block_factory_args", dict, config_dict, "architecture.temporal_encoder", {}
        )
        input_size = get_config_attr(
            "input_size", None, config_dict, "architecture.temporal_encoder", required=True
        )
    ...
    def compile(self) -> "TemporalEncoder":
        """Compile temporal encoder."""
        block_factory = encoder_decoder.get_block_factory(self.block_factory)
        block_factory = block_factory(**self.block_factory_args)
...
class DirectTemporalEncoder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        blocks = []
        ch_in = channels[0] + n_times
        for ch_out in channels[1:]:
            blocks += [
                block_factory(ch_in, ch_out),
                metnet.SpatialAggregator(input_size, ch_out, 1, 4),
            ]
            ch_in = ch_out
...
class DirectForecastConfig:
    ...
    def parse(
        ...
        forecast_range = get_config_attr("forecast_range", int, arch_config, "architecture", required=True)
        ...
        temporal_encoder_cfg = TemporalEncoderConfig.parse(latent_dim, n_times,  temporal_encoder_cfg)
    ...
    def compile(self) -> "DirectForecast":
        """
...
class DirectForecast(RetrievalModel):
    """
    ...
    def from_config_dict(cls, config_dict):
        ...
        config = DirectForecastConfig.parse(input_config, output_config, arch_config)
        return DirectForecast(input_config=input_config,
    ...
    def __init__(
            ...
            arch_config: Dict[str, DirectForecastConfig]
    ):
        super().__init__(
            config_dict={
                "input": {
        ...
        self.forecast_range = arch_config.forecast_range
    ...
    def forward(self, x):
        ...
        lead_times = x.get("lead_time")
        batch_size, sequence_length = lead_times.shape

        lead_times = encode_times(
            lead_times,
            self.time_step,
            self.forecast_range
        )
        ...
        encs = torch.cat(self.encoder(x), 1)

        encs = encs.repeat_interleave(sequence_length, 0)
        lead_times = torch.broadcast_to(
            lead_times[..., None, None], lead_times.shape + encs.shape[2:]
        )
        encs = torch.cat([encs, lead_times], 1)
        encs = self.temporal_encoder(encs)

        preds = self.decoder(encs)

        unpacked = {}
        for key, tensor in preds.items():
            shape = (batch_size, sequence_length) + tensor.shape[1:]
            unpacked[key] = list(tensor.reshape(shape).transpose(0, 1))
        return unpacked
